...
class AudioUnitMIDISynth: NSObject {
    ...
    func playPatch1On(n:UInt32 , vol:Int) {
        ...
        status = MusicDeviceMIDIEvent(self.midisynthUnit!, pcCommand, patch1, 0, 0)
        AudioUtils.CheckError(status)
        status = MusicDeviceMIDIEvent(self.midisynthUnit!, noteCommand, pitch, UInt32(vol), 0)
        AudioUtils.CheckError(status)
    }
    ...
    func playChord(n:UInt32, v:Array<Int>, vol:Int){
        ...
        if( !globalVar.arpeggio ){
            for a in v{
                status = MusicDeviceMIDIEvent(self.midisynthUnit!, noteCommand, pitch+UInt32(a), UInt32(vol), 0)
            }
        } else {
            var i:Int=0
            let n=DispatchTime.now()
            for a in v{
                DispatchQueue.main.asyncAfter(deadline: n + .milliseconds(60 * i)) {
                    status = MusicDeviceMIDIEvent(self.midisynthUnit!, noteCommand, self.pitch+UInt32(a), UInt32(vol), 0)
                }
                i+=1
            }
        }
